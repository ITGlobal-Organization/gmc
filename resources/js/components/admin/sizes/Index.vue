            ...
            <div class="row">
                <div class="col-12 col-md-5 card card-box-shadow">
                    <div class="card-header">
                        <h5>{{ Lang.create_msg(Lang.size) }}</h5>
                    </div>
                    <div class="card-body">
...
mounted(){
    ...
    this.FormFields = [
            {
                label:Language.name+" (example[widthXheight])",
                field:"name",
                class:"form-control",
    ...
    this.getSizes(this.pageNo,this.pageSize,this.filter);
},
...
methods:{
    ...
    async updateStatus(id){
        ...
        let ref = this
            confirmAlert(async () => {
                this.loader = true
                    const { udpateStatus } = useSizes();
                    await udpateStatus(id)
                    ref.getSizes(ref.pageNo,ref.pageSize,ref.filter);
            })
    },
    async _delete(id){
        ...
        let ref = this
            confirmAlert(async () => {
                this.loader = true
                    const { _delete } = useSizes();
                    await _delete(id)
                    ref.getSizes(ref.pageNo,ref.pageSize,ref.filter);
                    successAlert(Language.success_msg.replace(':attribute',Language.size).replace(':action',Language.deleted))
    ...
    async update(){
        ...
        this.loader =true;   
                await update(this.id,this.FormData).then((response) => {
                    successAlert(Language.success_msg.replace(':attribute',Language.size).replace(':action',Language.updated));
                    ref.id = 0;
                    ref.getSizes();
                }).catch((e) => {
    ...
    async store(){
            ...
            const {store,errors} = useSizes();
            const {successAlert,errorAlert} = useService();
            this.loader =true;
            let ref = this
            await store(this.FormData).then(async (response) => {
                successAlert(Language.success_msg.replace(':attribute',Language.size).replace(':action',Language.saved))
                this.getSizes();
            }).catch((e) => {
    ...
    async edit(id){
        const {record,get} = useSizes();
        this.id = id;
        await get(id);
        console.log(record.value);
        this.FormData.name = record.value.name;
        this.FormData.width = record.value.width;
        this.FormData.height = record.value.height;
