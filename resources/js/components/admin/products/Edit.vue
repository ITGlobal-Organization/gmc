...
methods:{
    async update(){
        const {update,errors} = useProducts();
    ...
    async edit(){
            const {record,get} = useProducts();
            
            await get(this.id);
            
            this.FormData.name = record.value.name;
            this.FormData.slug = record.value.slug;
            this.FormData.category_id = record.value.category_id;
            this.FormData.product_code = record.value.product_code;
            ...
            let Sizes = []
            let ref = this;
            if(record.value.product_digitals.length > 0){
                record.value.product_digitals.map(size => {
                    delete size['product_id']
                    ref.FormData.sizes.push(size)
                })
            }
