            ...
            <div class="card-header">
                ...
                <div class="row">
                    ...
                    <div class="col-4 d-flex justify-content-end">
                        <div class="row">
                            <div class="col-12">
                                <router-link class="btn btn-secondary" to="/admin/platinumpartners/create">
                                    {{ Lang.add_new_msg(Lang.platinumpartner) }}
                                </router-link>
...
export default {
  ...
  components: { Datatables,VueElementLoading,Select2 },
    ...
    mounted(){
                ...
                dropdown:[
                    {
                        ...
                        action:(row) =>{
                            ref.$router.push('/admin/platinumpartners/edit/'+row.id)
                        }
                    },
                    {
                        name:(row) => {
        ...
        this.getPlatinumPartners(this.pageNo,this.pageSize,this.filter);
    },
    ...
    methods:{
        ...
        async getPlatinumPartners(){

            const { records, getAll } = usePlatinumPartner();
            this.loader = true
            let ref = this
        ...
        async updateStatus(){
            ...
            let ids = []
                ...
                if(ids.length > 0) {
                    confirmAlert(async () => {
                    this.loader = true
                        const { updateStatus } = usePlatinumPartner();
                        await updateStatus({
                            ids:ids,
                            status:ref.approved
                        }).then((response) => {
                            successAlert(Language.success_msg.replace(':attribute',Language.platinumpartners).replace(':action',Language.updated));
                            ref.loader = false;
                            ref.getPlatinumPartners(ref.pageNo,ref.pageSize,ref.filter);
                        }).catch((e) => {
                            console.log(e)
                        ref.loader = false;
        ...
        async _delete(id){
            ...
            let ref = this
                confirmAlert(async () => {
                    this.loader = true
                        const { _delete } = usePlatinumPartner();
                        await _delete(id)
                        ref.getPlatinumPartners(ref.pageNo,ref.pageSize,ref.filter);
                        successAlert(Language.success_msg.replace(':attribute',Language.platinumpartner).replace(':action',Language.deleted))
