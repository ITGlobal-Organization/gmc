<template>
        ...
        <div class="card card-secondary card-outline">
            <div class="card-header d-flex justify-content-end">
                <router-link class="btn btn-secondary" to="/admin/orders/create">
                    {{ Lang.add_new_msg(Lang.order) }}
                </router-link>
  ...
  components: { Datatables,VueElementLoading },
    ...
    mounted(){
        ...
        this.columns = [
...

            {
                ...
                field:'employee',
                orignal_name:'employee',
                width:'4%',
                sorted:false,
                has_html:false,
            },
            {
                label:Language.order_date,
                field:'order_date',
                orignal_name:'order_date',
                width:'4%',
                sorted:false,
                has_html:false,
            },
            {
                label:Language.order_code,
                field:'order_code',
                orignal_name:'order_code',
                width:'4%',
                sorted:false,
                has_html:false,
            },
              {
                label:Language.total_price,
                field:'total_price',
                orignal_name:'total_price',
                width:'4%',
                sorted:false,
                has_html:false,
            },
            ...
            {
                label:Language.order_platform,
                field:'order_platform',
                orignal_name:'order_platform',
                width:'4%',
                sorted:false,
                has_html:false,
            },
            {
                label:Language.quoted_price,
                field:'quoted_price',
                orignal_name:'quoted_price',
                width:'4%',
                sorted:true,
                has_html:false,
            },
                ...
                dropdown:[
                    {
                        ...
                        action:(row) =>{
                            ref.$router.push('/admin/orders/edit/'+row.id)
        ...
        this.getOrders(this.pageNo,this.pageSize,this.filter);
    },
    ...
    methods:{
        ...
        async getOrders(){
            ...
            const { records, getAll } = useOrders();
            this.loader = true
            let ref = this
        ...
        async _delete(id){
            ...
            let ref = this
                confirmAlert(async () => {
                    this.loader = true
                        const { _delete } = useOrders();
                        await _delete(id)
                        ref.getOrders(ref.pageNo,ref.pageSize,ref.filter);
