<select class="pd-sort sort_by">
        ...
        <option value="created_at-desc" {{ ($orderBy=="created_at" && $order=="desc")? 'selected':''}}>{{ trans('lang.latest_to_oldest') }}</option>
        <option value="created_at-asc" {{ ($orderBy=="created_at" && $order=="asc")? 'selected':''}}>{{ trans('lang.oldest_to_latest') }}</option>
    </select>