...
class EventCalender extends BaseModel
{
    ...
    public static function boot()
    {
        ...
        static::addGlobalScope('active_events', function (Builder $builder) use ($table,$today,$after) {
            $user = Auth::user();
            if(isset($user) && $user->hasRole('admin')){
                ...
                $builder->where($table.'.is_delete', '=', 0)->where($table.'.is_active','=',1)->where($table.'.is_approved',1)->where('event_date','>=',$today)->where('event_date','<=',$after);
            }
        });
    }
